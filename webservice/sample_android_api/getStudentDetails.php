...
if (isset($_GET["id"])) {
    ...
    $result = mysqli_query($con, "SELECT *, (SELECT city_name FROM city as c WHERE c.city_id = st.st_city) as cityname, (SELECT zone_name FROM zone as z WHERE z.zone_id = st.st_zone) as zonename, (SELECT area_name FROM subarea as s WHERE s.area_id = st.st_subarea) as subareaname FROM students as st WHERE st_id='$id'");
